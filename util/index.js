const Util = {
  ...
  checkCardAtDispensePosition: () =>
    Buffer.from([0x2, 0x0, 0x2, 0x32, 0x30, 0x3, 0x1]),

  checkIfBoxIsEmpty: () => Buffer.from([0x2, 0x0, 0x2, 0x32, 0x30, 0x3, 0x1]),

  checkIfBoxIsPreEmpty: () =>
    Buffer.from([0x2, 0x0, 0x2, 0x32, 0x30, 0x3, 0x1]),

  checkCardReadPostion: () =>
    Buffer.from([0x2, 0x0, 0x2, 0x32, 0x30, 0x3, 0x1]),

  checkCardDispensePostion: () =>
    Buffer.from([0x2, 0x0, 0x2, 0x32, 0x30, 0x3, 0x1]),
  ...
  handleLinuxPorts: (port) => `/dev/pts/${port}`,
};
